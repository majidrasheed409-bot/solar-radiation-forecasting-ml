...
All pipeline code is inlined here – no src/ imports needed.
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Lags
    for var in ["GHI_kWh_m2","DNI_kWh_m2","DHI_kWh_m2","Temperature_C","Humidity_%","Wind_Speed_m_s"]:
    ...
    if "Temperature_C" in df.columns:
        df["Temp_Range_7d"] = df["Temperature_C"].rolling(7).max() - df["Temperature_C"].rolling(7).min()
    ...
    if "DNI_kWh_m2" in df.columns and "DHI_kWh_m2" in df.columns:
        df["DNI_DHI_Ratio"] = np.where(df["DHI_kWh_m2"] > 0, df["DNI_kWh_m2"] / df["DHI_kWh_m2"], 0.0)
        df["GHI_DNI_Ratio"] = np.where(df["DNI_kWh_m2"] > 0, df["GHI_kWh_m2"] / df["DNI_kWh_m2"], 0.0)

    extra = 1367.0 * (1 + 0.033 * np.cos(2 * np.pi * doy / 365))
    df["Clearness_Index"] = np.clip(np.where(extra > 0, df["GHI_kWh_m2"] / extra, 0.0), 0, 1)
    if "DHI_kWh_m2" in df.columns:
        df["Diffuse_Fraction"] = np.clip(np.where(df["GHI_kWh_m2"] > 0, df["DHI_kWh_m2"] / df["GHI_kWh_m2"], 0.0), 0, 1)
...
def evaluate_metrics(y_true, y_pred, model_name="", country="", feature_set=""):
    ...
    rating = "Excellent" if r2 >= 0.95 else "Good" if r2 >= 0.90 else "Acceptable" if r2 >= 0.80 else "Poor"
...
@st.cache_data
def generate_demo_data(country: str, n: int = 750) -> pd.DataFrame:
    ...
    if country == "Senegal": ghi *= 1.05
    if country == "Ghana":   ghi *= 0.93
    return pd.DataFrame({
        "Date":                    dates,
        "GHI_kWh_m2":             ghi,
...
def train_model(country: str, model_type: str, feature_set: str):
    ...
    feats  = [f for f in fs_map[feature_set] if f in train.columns and f != TARGET]
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("#### 📊 Pages")
    page = st.radio("", ["🏠 Overview", "📈 Forecast", "🔍 Feature Analysis",
                         "🗺️ Geographic", "🔮 Predict New Data"],
                    label_visibility="collapsed")
    st.markdown("---")
    st.markdown('<p style="color:#8B949E;font-size:0.72rem;line-height:1.6;">Data: World Bank energydata.info<br>Period: Sep 2021 – Nov 2023</p>', unsafe_allow_html=True)
...
if page == "🏠 Overview":
    ...
    bench = pd.DataFrame({
        "Study":  ["This study","Pinar et al. 2020","Voyant et al. 2017 (upper)","Voyant et al. 2017 (avg)","Yang et al. 2019"],
    ...
    fig3.update_traces(texttemplate="%{text:.4f}", textposition="outside", textfont_color="#E6EDF3")
    fig3.update_layout(**PLOTLY_LAYOUT, height=300, coloraxis_showscale=False, yaxis_range=[0.6,1.05])
...
# ============================================================
   ...
   fig = make_subplots(rows=2, cols=1, shared_xaxis=True, vertical_spacing=0.06,
    ...
    fig.add_trace(go.Scatter(x=test["Date"], y=test[TARGET], name="Test (actual)",
                              line=dict(color="#58A6FF",width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=test["Date"].values[-len(y_pred):], y=y_pred,
                              name="Predicted", line=dict(color="#F5A623",width=2,dash="dot")), row=1, col=1)
    residuals = y_test[-len(y_pred):] - y_pred
    fig.add_trace(go.Bar(x=test["Date"].values[-len(y_pred):], y=residuals,
                          name="Residuals",
    ...
    with col2:
        ...
        fig_qq.add_trace(go.Scatter(x=osm, y=osr, mode="markers",
                                     marker=dict(color="#58A6FF",size=4), name="Residuals"))
    ...
    monthly = tc.groupby(tc["Date"].dt.month).agg(
        Mean_GHI=(TARGET,"mean"), Mean_Pred=("Predicted","mean"), MAE=("AbsError","mean")
    ).reset_index()
...
# ============================================================
    ...
    if fi is None:
        ...
        for name, val in fi.items():
            n = name.lower()
            if "lag" in n:   cats["Lag"] += val
            elif "roll" in n: cats["Rolling"] += val
            elif any(k in n for k in ("month","year","quarter","season","sin","cos","day")):
                cats["Temporal"] += val
            else: cats["Interaction/Base"] += val

        col1, col2 = st.columns(2)
        with col1:
            fig_pie = go.Figure(go.Pie(
                labels=list(cats.keys()), values=list(cats.values()), hole=0.45,
                marker_colors=["#F5A623","#3FB950","#58A6FF","#BC8CFF"],
                textfont_color="#E6EDF3",
            ))
            fig_pie.update_layout(**PLOTLY_LAYOUT, height=300, title="Importance by category")
        ...
        with col2:
            ...
            fig_c.update_layout(**PLOTLY_LAYOUT, height=300, title="Cumulative importance",
                                xaxis_title="Features", yaxis_title="Cumulative", yaxis_range=[0,1.05])
...
# ============================================================
    ...
    fig_geo.update_layout(**PLOTLY_LAYOUT, height=300, yaxis_range=[0.85,1.0],
                          yaxis_title="R² Score",
...
# ============================================================
    ...
    st.markdown('<div class="section-head">Predict GHI from New Data</div>', unsafe_allow_html=True)
    tab1, tab2 = st.tabs(["📤  Upload CSV", "✏️  Manual Entry"])

    with tab1:
        st.markdown("Upload a CSV with daily meteorological observations (same columns as the training data).")
        ...
        if uploaded:
            try:
                ...
                if st.button("Run prediction"):
                    ...
                    fs_map = get_feature_sets(df_eng)
                    feats  = [f for f in fs_map[feature_set] if f in df_eng.columns and f != TARGET]
                    ...
                    if TARGET in df_eng.columns:
                        ...
                        m = evaluate_metrics(df_eng[TARGET].values, preds)
    ...
    with tab2:
        ...
        if st.button("🔮  Predict GHI"):
            ...
            today_row = pd.DataFrame([{
                ...
                "DNI_kWh_m2": dni, "DHI_kWh_m2": dhi,
                "Temperature_C": temp, "Temp_Max_C": temp+4, "Temp_Min_C": temp-4,
                "Humidity_%": hum, "Wind_Speed_m_s": wind, "Wind_Direction_deg": wdir,
                "Barometric_Pressure_hPa": pres, "Precipitation_mm": precip,
                "Country": cfg[0],
            }])
            ...
            fs_map   = get_feature_sets(df_eng)
            feats    = [f for f in fs_map[feature_set] if f in df_eng.columns and f != TARGET]
            X_last   = results["scaler"].transform(df_eng[feats].fillna(0).iloc[[-1]])
            ...
            fig_g = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    "bar": {"color":"#F5A623"}, "bgcolor":"#161B22",
            ...
            fig_g.update_layout(paper_bgcolor="rgba(0,0,0,0)", font_color="#E6EDF3",
